...
**Upstream:** `gbergeret/AI-Agent-Demo` - the repo this demo was copied from. If you
...
- **In:** framework / scaffolding files - playbooks, `CLAUDE.md`, `README.md`,
  `scripts/`, the `_TEMPLATE.md` files, `context/INDEX.md`, and the governance files
  (`context/GOLDEN-RULES.md`, `context/PRINCIPLES.md`).
- **Out (never auto-changed):** your personal content - `MEMORY.md`,
  `context/PROFILE.md`, `context/VOICE.md`, and everything under `projects/`. These
  are yours: the playbook shows you if they differ from upstream, but never
  overwrites them unless you say so for that specific file.
...
2. **Clone the latest upstream.** Clone it into a temporary folder *outside* this repo,
   ...
   `tmp="$(mktemp -d)"; git clone https://github.com/gbergeret/AI-Agent-Demo "$tmp"`.
...
4. **Copy the updates in - framework files only.** Copy the newer framework / scaffolding
   ...
   your personal files (`MEMORY.md`, `context/PROFILE.md`, `context/VOICE.md`,
   `projects/`). If you *also* changed a framework file locally, flag it and let the
   person decide what wins. Never delete a file that exists locally but not upstream - it
   may be their own.
...
- Never touch a personal file (`MEMORY.md`, `context/PROFILE.md`,
